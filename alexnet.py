...
class VGG16(nn.Module):
    ...
    def __init__(self, num_classes=7):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(30, 64, kernel_size=11, stride=4, padding=2),
    ...
    def forward(self, x):
        ...
        for k in range(10):
            f = x[:,k,:,:,:] #(-1,112,112,3)
            f = f.permute(0,3,1,2) #(-1,3,112,112)
            inp.append(f)
        inp = torch.cat(inp, axis=1) #(-1,30,112,112)
...
#             param_group['lr'] = 1e-5
    ...
    if (epoch+1) % 10 == 0 :
        ...
        torch.save(saved_dict, './RAFDB/alexnet112epoch{}.pkl'.format(epoch))
